...
/// <summary>
/// 统一 Command 后端 — Browser 和 Native 的实现完全相同。
/// 仅字段类型和构造函数因平台而异。
/// </summary>
public sealed unsafe class CommandBackend : ICommandBackend
{
#if BROWSER
    private readonly Dumb.Emscripten.WGPUBrowser _wgpu;

    public CommandBackend(Dumb.Emscripten.WGPUBrowser wgpu) => _wgpu = wgpu;
#else
    private readonly WebGPU _wgpu;

    public CommandBackend(WebGPU wgpu) => _wgpu = wgpu;
#endif
